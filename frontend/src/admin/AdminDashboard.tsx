...
export default function AdminDashboard() {
  ...
  const [dealRequestsCount, setDealRequestsCount] = useState<number | null>(
    null,
  );
  ...
  const [tripRequestsCount, setTripRequestsCount] = useState<number | null>(
    null,
  );
  ...
  const go = (v: string) => {
    ...
    if (typeof window !== "undefined")
      window.history.pushState({}, "", `/admin/${v === "home" ? "" : v}`);
  };
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      <aside
        className={`${sidebarOpen ? "w-64" : "w-16"} bg-slate-900 transition-all duration-300 flex flex-col`}
      >
